...
if api_key:
    ...
    # If not set in environment/dotenv, fallback to empty string
...
# Locate the Excel file in the same directory as the script
script_dir = os.path.dirname(os.path.abspath(__file__))
file_path = os.path.join(script_dir, "workaw_data.xlsx")

try:
    df = pd.read_excel(file_path)
    file_content = df.to_string(index=False)
except Exception as e:
    st.error(f"Error reading file: {e} (Path tried: {file_path})")
    st.stop()
    ...
    def generate_response():
        ...
        if prompt.lower().startswith("add") or prompt.lower().endswith("add"):
            ...
            st.session_state["messages"].append(
                {"role": "model", "content": "ขอบคุณสำหรับคำแนะนำค่ะ"}
            )
        else:
